...
import "../components/ui/accountCard.css";
...
const Dashboard = ({ user }: any) => {
	...
	const phoneNumber = user.contactInfo ||
		JSON.stringify(user.contactInfo['phone']) ||
		JSON.stringify(user.contactInfo['cell']) ||
		'';
		...
		<div className="dashboard-container">
			...
			<div className="account-card">
				...
				<div className="account-info">
					...
					<div>
						<span>Phone:</span>
						<span>{cleanedPhoneNumber}</span>
					</div>
					<div>
						<span>Tax Info:</span>
						<span>{user.taxInfo ? user.taxInfo.replace(/.(?=.{4})/g, '*') : '-'}</span>
					</div>
					<div>
						<span>Bank Info:</span>
						<span>{user.bankingInfo ? user.bankingInfo.replace(/.(?=.{4})/g, '*') : '-'}</span>
					</div>
				</div>
